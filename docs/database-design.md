...
Accepted [D-01–D-05](requirements/decisions.md) refine these contracts: OpenAI GPT-4o mini with extraction-v1; explicit same-position candidate confirmation before new-version upload; months-v1 with frozen as_of_date; and final decisions with no undo/switch. Store normalization_version, as_of_date and supported_months in resume_snapshots.extraction. The decision transaction must check status=scored before a first write, return unchanged data for a current-version identical retry, and reject a different final decision without incrementing version or replacing its timestamp.
