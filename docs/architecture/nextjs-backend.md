...
The implementation delivered for checklist item 7 uses the in-memory repositories and mock AI service accepted for this milestone — no PostgreSQL, no real AI calls, no deploy. The `resume_extraction_jobs` and `screening_runs`/`screening_run_items` claim/lease fields, and the `@app/worker` task modules (`tasks/extract-resume`, `tasks/run-screening`, `tasks/rescore`) that claim and drive them, are real, independently unit-tested code and match the shape this document describes. What is **not** yet true of the running system: the module-level in-memory store is process-local, so a worker started as its own `node` process would see an empty store, not the API process's data. Until a real PostgreSQL adapter backs both containers, the two-process topology above is a target, not the deployed reality — the poll loop in `backend/worker/src/index.ts` is only meaningful when driven in-process (e.g. from a test, or from the same process as the Next.js server).
