...
This is an explicit single-process exception for the mock milestone, not the durable two-process topology described above. Starting the worker CLI separately sees an empty store. Process restart loses all mock data and queued work; serverless and multiple API processes are unsupported until shared persistent adapters are implemented. The eight deferred API operations and the new frontend remain outside this change.
