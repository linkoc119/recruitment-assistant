...
In the current implementation, unexpected handler failures return the standard `500 internal_error` envelope. The corresponding `http_request_failed` event records only that code and the response request ID. Worker failures record `worker_task_failed`, a fixed `unexpected_worker_error` code and task/job/resume/run identifiers where available; original messages, stacks and arbitrary exception properties are not serialized. Expected item failures retain their safe error code and phase in the run/extraction state. Synthetic leakage regression tests cover the logging boundaries; see [Phase 3 verification](../ui-ux/phase-3-verification.md) for application-level evidence and scope limits.
