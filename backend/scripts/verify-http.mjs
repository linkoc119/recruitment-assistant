...
// History, identity selection, frozen source/file and comparison endpoints.
const revisions = await request(`${root}/criteria-revisions`);
assert.deepEqual(revisions.items.map(r => r.revision), [1, 2]);
assert.equal((await request(`${root}/criteria-revisions/1`)).criteria[0].label, "TypeScript");
assert.equal((await request(`${root}/candidates`)).page.total, 2);
const history = await request(`${root}/screening-runs?published_only=true`);
assert.deepEqual(history.items.map(r => r.id), [rescore.id, run.id]);
assert.equal(history.items[1].is_current, false);
const preview = await request(`${resultPath}/source`);
assert.equal(preview.raw_text, cvTexts[0]);
assert.equal(preview.snapshot_id, detail.snapshot_id);
const originalFile = await fetch(`${base}/api${resultPath}/file`);
assert.equal(originalFile.status, 200);
assert.equal(originalFile.headers.get("cache-control"), "no-store");
assert.equal(originalFile.headers.get("x-content-type-options"), "nosniff");
assert.equal(originalFile.headers.get("content-type"), "application/pdf");
assert.ok(originalFile.headers.get("x-request-id"));
assert.match(originalFile.headers.get("content-disposition"), /^inline;/);
assert.equal(await originalFile.text(), cvTexts[0]);
const diff = await request(`${root}/comparison?left_run_id=${run.id}&right_run_id=${rescore.id}`);
assert.deepEqual(diff.items.map(i => i.score_delta), ["-100.00", "100.00"]);
assert.deepEqual(diff.items.map(i => i.rank_delta), [1, -1]);
assert.ok(diff.changed_criteria.some(c => c.change === "modified" && c.fields.includes("skill_id")));
assert.equal((await write(`${root}/resumes/${resumeIds[0]}/reprocess`, undefined, 409)).code, "request_in_progress");
console.log(`HTTP workflow passed: job ${job.id}, initial ${run.id}, rescore ${rescore.id}; main flow, history, candidates, source/file, comparison and reprocess guard verified.`);
