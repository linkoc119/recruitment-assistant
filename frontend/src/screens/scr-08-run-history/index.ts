...
import { runStatusLabel } from "../../lib/html.js";
const view=screen(async v=>{
 ...
 table(["SELECT","RUN","MODE","CRITERIA","COMPLETED","OUTCOME","STATUS","ACTIONS"],runs.map(r=>`<tr><td><input type="checkbox" class="run-select" value="${r.id}" aria-label="Select run ${r.id}" ${r.published_at?"":"disabled"}></td><td>Run ${r.id} · Round ${r.round}</td><td>${esc(r.mode)}</td><td>${link(`${ui}/criteria/${r.criteria_revision}`,`Revision ${r.criteria_revision}`)}</td><td>${esc(r.finished_at?new Date(r.finished_at).toLocaleString():"—")}</td><td>${r.counts.succeeded}/${r.counts.total} scored · ${r.counts.failed} errors</td><td>${badge(r.is_current?"Current":runStatusLabel(r.status))}</td><td>${link(r.published_at?`${ui}/ranking?run=${r.id}`:`${ui}/screening-runs/${r.id}`,r.published_at?"View Results":"View Progress")}</td></tr>`).join("")));
